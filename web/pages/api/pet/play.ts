...
/**
 * POST /api/pet/play - Play with the user's ghost pet
 * Updates mood to 100, adds XP, and levels up if threshold reached
 */
export default async function handler(
  ...
  try {
    ...
    return sendSuccess(
      ...
      200,
      leveledUp ? `Played! Your ghost leveled up to ${newLevel}! 🎉` : 'Played! Your ghost is cheerful! 😊'
    )
  } catch (error) {
    console.error('Play with pet error:', error)
    return sendServerError(res, 'Failed to play with pet')
  }
}
